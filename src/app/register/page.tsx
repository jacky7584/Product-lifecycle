...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl
        .fromTo('.register-title', { opacity: 0, y: -20 }, { opacity: 1, y: 0, duration: 0.5 })
        .fromTo('.register-form', { opacity: 0, y: 30, scale: 0.97 }, { opacity: 1, y: 0, scale: 1, duration: 0.6 }, '-=0.2')
        .fromTo('.register-fields > div', { opacity: 0, x: -20 }, { opacity: 1, x: 0, duration: 0.4, stagger: 0.1 }, '-=0.3')
        .fromTo('.register-submit', { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.3 }, '-=0.1')
        .fromTo('.register-footer', { opacity: 0 }, { opacity: 1, duration: 0.3 }, '-=0.1')
    }, formRef)
  ...
  useEffect(() => {
    ...
    const form = formRef.current.querySelector('.register-form')
    if (form) {
      gsap.fromTo(form,
        { x: -8 },
  ...
  return (
    <div ref={formRef} className="min-h-[60vh] flex items-center justify-center" style={{ visibility: 'hidden' }}>
      <div className="w-full max-w-sm">
        <h1 className="register-title text-2xl font-bold text-text-primary text-center mb-6">註冊 Register</h1>
        <form onSubmit={handleSubmit} className="register-form bg-bg-default rounded-lg border border-border-primary p-6 space-y-4">
          <div className="register-fields space-y-4">
            <div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="register-submit w-full px-4 py-2 text-sm font-medium text-text-onbrand bg-bg-brand rounded-md hover:bg-bg-brand-compliment disabled:opacity-50 transition-colors min-h-[44px] cursor-pointer focus-visible:ring-2 focus-visible:ring-border-focus focus-visible:ring-offset-2"
          >
            {submitting ? '註冊中...' : '註冊'}
          </button>
        </form>
        <p className="register-footer text-sm text-text-tertiary text-center mt-4">
          已有帳號？{' '}
